...
const itemsData = filter =>{
    console.log(filter)
    //filter será enviado ao backend
    const itemsRetur =  [
        {
    ...
    if(itemsRetur.length < 11){
        for(let i = itemsRetur.length; i < 11; i++){
            itemsRetur.push({status_order:'¨'})
        }
    }
...
const itemsModify = (itemsDatas) =>{
    itemsDatas.forEach(element => {
        ...
        if(element.status_order == 'Parado'){
...
const items = filter => itemsModify(itemsData(filter))
